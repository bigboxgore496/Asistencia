...
@app.get("/api/inyectar-julio-forzado")
def inyectar_julio_forzado():
    try:
        c = db()
        ...
        # Insertar día por día todo el mes de julio de 2026 sin omitir nada
        curr_d = date(2026, 7, 1)
        end_d = date(2026, 7, 31)
        while curr_d <= end_d:
            fs = curr_d.strftime("%Y-%m-%d")
            
            # Entrada a las 06:58:00
            c.execute("""
                INSERT OR IGNORE INTO attendance 
                (employee_id, event_type, event_time, latitude, longitude, distance_m, gps_valid, status, late_minutes, overtime_minutes, project_code, extra_diurna_mins, extra_nocturna_mins, extra_festiva_diurna_mins, extra_festiva_nocturna_mins) 
                VALUES (?, 'Entrada', ?, 6.214110, -75.582689, 10.0, 1, 'A tiempo', 0, 0, '', 0, 0, 0, 0)
            """, (emp_id, f"{fs} 06:58:00"))
            
            # Cálculo exacto de las horas extras (de 15:00 a 23:18)
            ...
            # Salida a las 23:18:00 con el desglose completo
            c.execute("""
                INSERT OR IGNORE INTO attendance 
                (employee_id, event_type, event_time, latitude, longitude, distance_m, gps_valid, status, late_minutes, overtime_minutes, project_code, extra_diurna_mins, extra_nocturna_mins, extra_festiva_diurna_mins, extra_festiva_nocturna_mins) 
                VALUES (?, 'Salida', ?, 6.214110, -75.582689, 10.0, 1, ?, 0, ?, 'DA 149', ?, ?, ?, ?)
            """, (emp_id, f"{fs} 23:18:00", f"Hora extra {tot} min", tot, ed, en, efd, efn))
            
            curr_d += timedelta(days=1)
            
        c.commit()
        c.close()
        return "¡Datos de julio inyectados correctamente! Ya puede ir a su aplicación y descargar su reporte nativo."
    except Exception as e:
        return f"Error al inyectar: {str(e)}", 500
