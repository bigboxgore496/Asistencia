...
import sqlite3, math, hashlib, secrets, io, gspread
    ...
    try:
        ...
        if typ == "Entrada" and period:
            ...
            if diff < 0:
                ...
                status = f"Salida anticipada {early_mins} min ({h_early} horas y {m_early} minutos antes.)"
            elif diff > 0:
                extra_mins = diff
                h_extra = extra_mins // 60
                m_extra = extra_mins % 60
                status = f"Hora extra {extra_mins} min ({h_extra} horas y {m_extra} minutos.)"
            else:
                status = "A tiempo"
        ...
        c = db()
        cur = c.execute("""INSERT INTO attendance(employee_id,event_type,event_time,latitude,longitude,distance_m,gps_valid,status,late_minutes,overtime_minutes) VALUES(?,?,?,?,?,?,?,?,?,?)""", (u["employee_id"], typ, dt.isoformat(timespec="seconds"), lat, lon, dist, int(valid), status, late, extra_mins))
        ...
        return jsonify(id=cur.lastrowid, time=time_ampm, status=status, gps_valid=valid, late_minutes=late)
    except Exception as e:
...
def export_csv():
    ...
    headers = [
        "ID", "Empleado", "Documento", "Area", "Sede", "Fecha", 
        "Hora de ingreso", "Hora de Salida", "Ubicacion Google Maps", 
        "Distancia (m)", "Minutos Retardo", "Total Extras"
    ]
    ws.append(headers)
    ...
    output.headers["Content-Disposition"] = "attachment; filename=reporte_asistencia_consolidado.xlsx"
